...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:mr.Sergio⚜️🦆⁩⁩\n' 
            + 'ORG:mr.Sergio⚜️🦆⁩;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=525623741080+52 56 2374 1080n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 30
...
/******** OWNER NUMBER**********/
const ownerNumber = ["525623741080@s.whatsapp.net","525623741080@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('Código listo','white'),color('CUACK','red'),color('SUSCRIBETE','white'),color('Mr.Sergio','yellow'))
})
           ...
           return false
			...
			switch(command) { 
                ...
                case 'glitchtext':
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Ejemplo : ${prefix}glitchtext Bot & Sergio`)
					reply(ind.wait())
				...
				case 'tahta':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Ejemplo : ${prefix}hartatahta Sergio`)
					har = body.slice(12)
					reply('「❗」Hartatahta')
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                var gh = body.slice(12)
					...
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'Mr.Sergio-Bot', quoted: mek})
					await limitAdd(sender)
					break
				//fadli 
				case 'pinterest':
               ...
               case 'help': 
         	   ...
         	   case 'owner':
         	   case 'creator':
                  client.sendMessage(from, {displayname: "Sergio", vcard: vcard}, MessageType.contact, { quoted: mek})
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
				...
				case 'slap':
					...
					client.sendMessage(from, pod, image, { quoted: mek, caption: '*Sergio*\n\n'+ ple })
					await limitAdd(sender)
					break
					case 'tampar':
					if (!isRegistered) return reply(ind.noregis())
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isQuotedSticker) return reply('Responde a un sticker')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'mr.Sergio -𝕭𝖔𝖙⚜️🦆⁩⁩'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
				break 
